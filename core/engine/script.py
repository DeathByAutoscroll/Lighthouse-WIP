...
class Script(gui.widget):
    ...
    def init(self,scene="",macros=True,ext=".txt",scriptlines=None):
        ...
        for i,line in enumerate(self.scriptlines):
            ...
            if line.startswith("list "):
                splitup = line.split(" ",1)[1].strip().split(' ')
                label_compat_splitup = []
                label_splitup = []
                for part in splitup:
                    if part == "noback":
                        label_compat_splitup.append(part)
                    elif not part.startswith("bg="):
                        label_compat_splitup.append(part)
                        label_splitup.append(part)
                label = '_'.join(label_splitup)
                label_compat = '_'.join(label_compat_splitup)
                self.labels.append([label, i-1])
                if label_compat != label: self.labels.append([label_compat, i-1])
    ...
    def safe_exec(self,method,*args):
        try:
            ...
            self.obs.append(error_msg("Undefined:"+str(e.message),self.lastline_value,self.si,self))
            import traceback
            traceback.print_exc()
        return e
    def update(self):
...
KEYWORD('name','The name of this timer so it can later be controlled. Only one timer with this name can be activated at once.')],type="gameflow")
    ...
    KEYWORD('present','whether to show the present button','false'),
    KEYWORD('move','whether to show the move button','false'),
    KEYWORD('fail','label to jump to if the label for an action was not found','none'),
    KEYWORD('bg','what to set the override background to','')],type="interface")
    def _lmenu(self,command,*args):
        """Show an investigation menu of options. Should be run after the background of a scene is loaded. When an option
        is clicked, PyWright will jump to the label of the action, such as "label examine" or "label talk". You can control which options
        are shown through the keywords described. This version doesn't clear the screen at all."""
        ...
        if splitargs.get('bg', None):
            assets.variables['script._override_bg'] = 'bg/' + splitargs['bg']
            del splitargs['bg']

        # See if there is an image defined...
        background = assets.variables.get("script._override_bg", assets.variables.get("_investigate_bg", ""))

        # This is a very janky hack.
        if background != "":
            self._clear('clear', 'bottom')
            self._obj('bg', "../" + background, "y=192", "stack", "z=-1")
        
        m = menu(custombg = background)
        if splitargs.get('fail', None):
            m.fail = splitargs['fail']
            del splitargs['fail']
        
        for arg,val in splitargs.items():
            if vtrue(val):
                m.addm(arg)
...
resume when the new script exits, otherwise, the current script will vanish."""
        ...
        # Firstly, reset the temporary variable...
        assets.variables.pop("script._override_bg", None)

        to_reset = []
        for var in assets.variables.keys():
            if (var.startswith('script.')):
                to_reset.append(var)
        for var in to_reset:
            del assets.variables[var]
...
set [tag]_check: name of a script to run if item is checked by player"""
    ...
    @category([VALUE("tag","Tag this list with some name so PyWright can keep track of which list options the player has tried already","Default is to name the list based on what script it's found in and on what line"),
               TOKEN("noback","No back button will be displayed if this token is included"),
               KEYWORD('bg','what to set the override background to','')],type="interface")
...
"""
        ...
        if bg:
            assets.variables['script._override_bg'] = bg
