...
set _font_new_resume_size 14""".split("\n"):
    ...
    def _open_art_(self,name,key=None):
        ...
        if not artpath:
            raise art_error("No image found:"+pre+name)
    ...
    def show_load(self):
        if not hasattr(self,"last_load"):
            self.last_load = time.time()-0.05
        if time.time()-self.last_load<0.05:
            return
...
class penalty(fadesprite):
    ...
    def draw(self,dest):
        v = self.gv()
        x = assets.sw-106
        dest.blit(self.left,[x,12]); x+=2
        if v<0: v=0
        for i in range(v):
            dest.blit(self.good,[x,12]); x+=1
        for i in range(100-v):
            dest.blit(self.bad,[x,12]); x += 1
        dest.blit(self.right,[x,12])
        if self.flash_amount:
            fx = assets.sw-104+v-self.flash_amount
...
class textbox(gui.widget):
    ...
    def __init__(self,text="",color=[255,255,255],delay=2,speed=1,rightp=True,leftp=False,nametag="\n"):
        ...
        self.color = color
        self.delay = delay
        self.basespeed = float(assets.variables.get("_textbox_speed_multiplier", 1))
        if assets.portrait:
            self.basespeed *= float(assets.variables.get("char_" + assets.portrait.charname + "_spdmult", 1))
        self.speed = speed * self.basespeed
        self.next_char = 0
    ...
    def forward(self,sound=True):
        ...
        if not self.text.strip():
            self.delete()
        if sound:
            continue_sound = assets.variables.get("_continue_sound","bloop.ogg")
            assets.play_sound(continue_sound, volume=0.7)
    ...
    def draw(self,dest):
        ...
        if self.nt_full:
            ...
            for ii in range(self.nt_text_image.get_width()+8):
                # TODO: maybe see if I can dynamically get the width of nt_left and and so build off of that for more accurate nt?
    ...
    #FIXME: VERY HACK
    def will_block(self):
        t = textbox(self.text,speed=0)
        t.update(0,run_code=False)
        print repr(t.mwritten)
        return len(t.mwritten)==1
    def add_character(self,run_code=True):
        command = None
        next_char = 1
        char = self._markup._text[len(self.mwritten)]
        self.mwritten.append(char)
        if isinstance(char,textutil.markup_command) and run_code:
            ...
            if assets.cur_script.macros.get(command,None):
                ...
                if command == "sfx":
                    ...
                    self.speed = self.basespeed * float(args)
            ...
            if char == " " or char == "\n":
                if self._lc == u'.':
                    next_char = int(assets.variables.get("_punctuation_delay_period","4"))
                if self._lc == u'?':
                    next_char = int(assets.variables.get("_punctuation_delay_question","4"))
                if self._lc == u"!":
                    next_char = int(assets.variables.get("_punctuation_delay_exclamation","8"))
                if self._lc == u",":
                    next_char = int(assets.variables.get("_punctuation_delay_comma","8"))
            if self._lc == u"-" and (char.isalpha() or char.isdigit()):
                next_char = int(assets.variables.get("_punctuation_delay_dash","4"))
            if char == u"(":
                self.in_paren = 1
            if char == u")":
                self.in_paren = 0
            if assets.portrait and run_code:
            ...
            if str(char).strip() and run_code:
                if (not vtrue(assets.variables.get("_limit_blip_speed","false"))) or (not self.blip_sound_channel) or (not self.blip_sound_channel.get_busy()):
                    self.blip_sound_channel = assets.play_sound(self.clicksound,volume=random.uniform(0.65,1.0))
    ...
    def update(self,dt=None,run_code=True):
        ...
        while (not self.nextline()) and self.next_char<=0:
            ...
            while (not self.nextline()) and ((not self.speed) or cnum>0):
                cnum -= 1
                ac_next = self.add_character(run_code=run_code)
                added += 1
                if self.speed:
                    next_char += ac_next
                    print("Increase wait:", ac_next, next_char)
            if self.speed:
                self.next_char += (next_char/float(self.speed))
                print("Set real next_char:", self.next_char)
        print("added chars to text", "chars:", num_chars, "speed:", self.speed, "actually_added:", added)
        #print(repr(self.mwritten))
        #print("ac_next", ac_next)
        if added: print("waiting:", ac_next, "dt:", dt, "".join([str(t) for t in self.mwritten]))
        if assets.portrait:
        ...
        if self.next==0:
            self.img = self.base.copy()
            y, stx, inc = 4, 6, 16*(assets.sh/standard_sh)
            x = stx
            color = self.color
            center = False
            t = textutil.markup_text()
...
class uglyarrow(fadesprite):
    def __init__(self):
        ...
        self.load(assets.variables.get("script._override_bg", assets.variables.get("_bigbutton_bg","bg/main")))
    ...
    def draw(self,dest):
        ...
        if self.button:
            ...
            self.double.pos[1] = (assets.sh-self.double.img.get_height())//2+assets.sh+9
            self.double.draw(dest)
            self.iwidth = self.double.img.get_width()
            self.iheight = self.double.img.get_height()
            if self.can_click():
                self.arrow.pos[0] = (assets.sw-self.arrow.img.get_width())//2-74
                self.arrow.pos[1] = (assets.sh-self.arrow.img.get_height())//2+assets.sh+9
...
class menu(fadesprite,gui.widget):
    ...
    def click_up(self,mp):
        ...
        if self.selected==o and o is not None:
            self.enter_down()
    def __init__(self,custombg=None):
        self.pri = ulayers.index(self.__class__.__name__)
        oy = 192
        ...
        if custombg == "":
            self.custombg = ""
        elif custombg and assets.num_screens == 2:
            #self.load(custombg)
            self.custombg = fadesprite(y=192).load(custombg)
        else:
            #self.load("bg/black")
            self.custombg = None
        self.bg = None
        ...
        self.opos = {"examine":[stx,sty],"move":[stx+self.opt.get_width()/2,sty],
            "talk":[stx,sty+self.opt.get_height()/2+1],"present":[stx+self.opt.get_width()/2,sty+self.opt.get_height()/2+1]}
        imgs = []
        x=y=0
    ...
    def delm(self,opt):
        if opt in self.options:
            ...
            if self.selected == opt:
                self.selected = None
    def draw(self,dest):
        if (not self.custombg) and (not self.bg):
            for o in reversed(assets.cur_script.obs):
                if isinstance(o,bg):
                    self.bg = o.img.copy()
                    break
        self.screen_setting = "try_bottom"
        if (not self.custombg) and self.bg:
            dest.blit(self.bg,self.getpos())

        if (self.custombg):
            self.custombg.draw(dest)
...
class listmenu(fadesprite,gui.widget):
    ...
    def __init__(self,tag=None):
        ...
        self.load(assets.variables.get("script._override_bg", assets.variables.get("_list_bg_image","general/black")))
        self.max_fade = int(float(assets.variables.get("_menu_fade_level",50)))
        self.fade = 0
...
class case_menu(fadesprite,gui.widget):
    ...
    def case_screen(self):
        ...
        if os.path.exists(os.path.join(self.path,self.options[self.choice],"case_screen.txt")):
            ...
            script = assets.cur_script.add_child_script("case_screen")
    def enter_down(self):
...
class evidence_menu(fadesprite,gui.widget):
    ...
    def __init__(self,items=[],gba=True):
        ...
        #Fix bad saved current page index
    ...
    def draw(self,dest):
        ...
        if not assets.gbamode or self.mode != "zoomed":
            dest.blit(assets.get_image_font("itemname").render(name,[248,168,24]),
